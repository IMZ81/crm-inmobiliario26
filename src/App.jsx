...
function fmtM2(n){ return n ? Number(n).toLocaleString("es-ES")+" m²" : "—"; }

// Calcula €/m² sobre m² útiles; si no, sobre construidos
function precioM2(lead) {
  const pvp = Number(lead.pvp_actual) || Number(lead.pvp_salida);
  const m2  = Number(lead.m2_util) || Number(lead.m2_const);
  if (!pvp || !m2) return "—";
  return Math.round(pvp / m2).toLocaleString("es-ES") + " €/m²";
}
...
export default function App() {
  ...
  async function fetchSheet() {
    ...
    } catch(e) { console.error("Error leyendo el Sheet:", e); }
    finally { setLoading(false); }
  }

  useEffect(() => {
    fetchSheet();
    const iv = setInterval(fetchSheet, 5*60*1000);
        ...
        try {
          await fetch(WEBHOOK_URL, {
            ...
            body: JSON.stringify({
              ...
              "FECHA SALIDA":      newLead.fecha_pub,
              "DOM":                newLead.dom,
  ...
  function advance(id) {
    setLeads(ls=>ls.map(l=>{
      if(l.id!==id) return l;
      const next=NEXT[l.stage]; return next?{...l,stage:next}:l;
    }));
  }

  function startEdit(lead) { setForm({...EMPTY,...lead}); setEditId(lead.id); setView("form"); }
  ...
  return (
    <div style={{fontFamily:"'Inter',sans-serif",background:"#0f172a",minHeight:"100vh",color:"#e2e8f0"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1e293b,#0f172a)",borderBottom:"1px solid #1e293b",padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <div style={{fontSize:11,color:"#64748b"}}>
            {loading?"Cargando…":lastSync?`Actualizado: ${lastSync.toLocaleTimeString("es-ES")}`:"Sin conexión"}
      ...
      {loading&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:200,color:"#64748b",fontSize:14}}>Conectando con Google Sheets…</div>}
                    ...
                    return (
                      <div key={lead.id} draggable onDragStart={()=>setDrag(lead.id)}
                        ...
                        <div style={{display:"flex",gap:6,marginBottom:4,flexWrap:"wrap"}}>
                          {lead.m2_const&&<span style={{fontSize:9,color:"#94a3b8"}}>🏗 {lead.m2_const}m²</span>}
                          {lead.m2_util&&<span style={{fontSize:9,color:"#94a3b8"}}>✅ {lead.m2_util}m²u</span>}
                          {precioM2(lead)!=="—"&&<span style={{fontSize:9,color:"#f59e0b",fontWeight:700}}>{precioM2(lead)}</span>}
                        </div>
        ...
        <div onClick={()=>setDetail(null)} style={{position:"fixed",inset:0,background:"#000000bb",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#1e293b",borderRadius:16,padding:24,maxWidth:520,width:"100%",maxHeight:"85vh",overflowY:"auto",border:"1px solid #334155"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 16px",fontSize:12}}>
              {[
                ["📍 Zona",          detailLead.zone],
                ["📞 Teléfono",      detailLead.phone],
                ["👤 Agente",        detailLead.agent],
                ["📅 Captación",     detailLead.date],
                ["📡 Canal",         `${chIcon[detailLead.channel]} ${detailLead.channel}`],
                ["🔖 Etapa",         STAGES.find(s=>s.id===detailLead.stage)?.label],
                ["💰 PVP Salida",    fmt(detailLead.pvp_salida)],
                ["💰 PVP Actual",    fmt(detailLead.pvp_actual)],
                ["🏗 m² Construidos",fmtM2(detailLead.m2_const)],
                ["✅ m² Útiles",     fmtM2(detailLead.m2_util)],
                ["📐 Precio €/m²",   precioM2(detailLead)],
                ["📆 Fecha Salida",   detailLead.fecha_pub||"—"],
                ["⏱ DOM",           detailLead.dom?detailLead.dom+"d":"—"],
                ["🎥 Vídeo",        detailLead.video],
                ["📐 Plano",        detailLead.plano],
              ].map(([k,v])=>(
      ...
      {view==="form"&&(
        <div style={{padding:20,maxWidth:560}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 12px"}}>
            ...
            {inp("m2_const","m² Construidos","number","ej. 360")}
            {inp("m2_util","m² Útiles","number","ej. 310")}
            {inp("link_anuncio","Link Anuncio")} {inp("link_dashboard","Link Dashboard")}
          </div>
          <div style={{marginBottom:8,background:"#0f172a",borderRadius:8,padding:"8px 12px",fontSize:11,color:"#f59e0b"}}>
            📐 Precio €/m²: <strong>{precioM2(form)}</strong> <span style={{color:"#475569"}}>(calculado automáticamente)</span>
          </div>
          <div style={{marginBottom:12}}>
